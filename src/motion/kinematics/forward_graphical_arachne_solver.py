...
class ForwardKinematicGraphicalArachneSolver:
    ...
    def solve(config: LegConfig, thetas: Tuple[float, float, float]) -> Tuple[float, float, float]:
        ...
        dst_y = math.cos(math.radians(alpha_1)) * inner_hypotenus
        diagonal_length = pythagorean_side(dst_y, inner_hypotenus) + coaxia_length

        dst_x = diagonal_length * math.sin(math.radians(theta_1))
        dst_z = pythagorean_side(x, diagonal_length)

        return __convert_quadrant__(theta_1, dst_x, dst_y, dst_z)
